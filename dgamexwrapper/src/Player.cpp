...
ResponseDef<PlayerNF> *PlayerNF::Responses;

CustomEventResponseSet<PlayerNF> PlayerNF::cerSet{};
void(__thiscall *PlayerNF::Respawn_Orignal)(Player*_this, Event *ev);
void(__thiscall *PlayerNF::Killed_Orignal)(Player*_this, Event *ev);
...
void PlayerNF::Init()
{
	...
	size_t sz = sizeof(ResponseDef<PlayerNF>) * rscount;
	//Responses = reinterpret_cast<ResponseDef<PlayerNF>*>(gi->Malloc(sz));
	Responses = new ResponseDef<PlayerNF>[rscount];
	...
	for (size_t i = 0; i < rscount-1; i++)
	{
		...
		const std::string str2 = typeid(ResponseDef<PlayerNF>::response).name();
		Responses[i].event = cerSet.GetEventAt(i);
		Responses[i].response = *cerSet.GetResponseFuncAt(i).target<void(PlayerNF::*)(Event*)>();
	}
	...
	ret = DetourAttach(&(PVOID&)(PlayerNF::Respawn_Orignal), (PVOID)(&Respawn));
	ret = DetourTransactionCommit();

	Killed_Orignal = reinterpret_cast<void(__thiscall *)(Player*_this, Event*)>((int)KILLED_ADDR);
	ret = DetourTransactionBegin();
	ret = DetourUpdateThread(GetCurrentThread());
	ret = DetourAttach(&(PVOID&)(PlayerNF::Killed_Orignal), (PVOID)(&Killed));
	ret = DetourTransactionCommit();
}
...
void PlayerNF::Shutdown()
{
	...
	LONG ret = DetourTransactionBegin();
	ret = DetourUpdateThread(GetCurrentThread());
	ret = DetourDetach(&(PVOID&)(PlayerNF::Respawn_Orignal), (PVOID)(&Respawn));
	ret = DetourTransactionCommit();


	ret = DetourTransactionBegin();
	ret = DetourUpdateThread(GetCurrentThread());
	ret = DetourDetach(&(PVOID&)(PlayerNF::Killed_Orignal), (PVOID)(&Killed));
	ret = DetourTransactionCommit();
}
...
void __fastcall Respawn(Player *_this, void * edx, Event * ev)
{
	...
	PlayerNF::Respawn_Orignal(_this, ev);
}
...
void __fastcall Killed(Player*_this, void * edx, Event * ev)
{
	...
	PlayerNF::Killed_Orignal(_this, ev);
}
