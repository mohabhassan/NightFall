...
typedef struct {} playerState_t; // empty defintion to avoid mistakes.
...
typedef struct {} gclient_t; //empty declaration
class GClient
{
	union GClientType
	{
	...
	{
		static client_persistant_t cpers = { 0 };
		if (!realGClient) return cpers;
		if (gameInfo.IsAA())
	...
	PlayerState initPS()
	{
		//PlayerState has its own null checks.
		if (gameInfo.IsAA())
...
class GEntity
{
	...
	entityState_t& init_s()
	{
		static entityState_t estate = { 0 };
		if (!realGent) return estate;
		if (gameInfo.IsAA())
		{
			return realGent->gentAA.s;
		}
		else if (gameInfo.IsSH())
		{
			return realGent->gentDSH.s;
		}
		else if (gameInfo.IsBT())
		{
			return realGent->gentDBT.s;
		}
	}
	gclient_t* const& init_client()
	{
		static gclient_t* gcp = { 0 };
		if (!realGent) return gcp;
		if (gameInfo.IsAA())
		{
			return (gclient_t*)realGent->gentAA.client;
		}
		else if (gameInfo.IsSH())
		{
			return (gclient_t*)realGent->gentDSH.client;
		}
		else if (gameInfo.IsBT())
		{
			return (gclient_t*)realGent->gentDBT.client;
		}
	}

	Entity* const& init_entity()
	{
		if (!realGent)
			return NULL;
		else if (gameInfo.IsAA())
		{
			return realGent->gentAA.entity;
		}
		else if (gameInfo.IsSH())
		{
			return realGent->gentDSH.entity;
		}
		else if (gameInfo.IsBT())
		{
			return realGent->gentDBT.entity;
		}
	}
public:
	GClient client;
	Entity*const& entity;//todo: remove void
	entityState_t& s;
	GEntity(gentity_t* gent)
		: realGent((realGentityType*)gent), s(init_s()), client(init_client()), entity(init_entity())
	{
	}

	bool isValid()
	{
		return realGent != NULL;
	}
...
typedef struct {} client_t; // placeholder for the datatype
...
class Client
{
	...
	#define CLIENT_MEMEBER_GETTER(varname) \
		decltype(clientAA_t::varname)& init_##varname() \
		{\
			static decltype(clientAA_t::varname) s_##varname;\
			if(!realClient) return s_##varname;\
			if (gameInfo.IsAA())\
			return realClient->clientAA.varname;\
					else if (gameInfo.IsSH())\
			return realClient->clientDSH.varname;\
					else if (gameInfo.IsBT())\
			return realClient->clientDBT.varname;\
		}

	CLIENT_MEMEBER_GETTER(userinfo);
	CLIENT_MEMEBER_GETTER(netchan);
	CLIENT_MEMEBER_GETTER(name);
	CLIENT_MEMEBER_GETTER(state);
	CLIENT_MEMEBER_GETTER(ping);
	CLIENT_MEMEBER_GETTER(rate);
	CLIENT_MEMEBER_GETTER(gentity);
#undef CLIENT_MEMBER_GETTER
public:
	...
	Client(client_t* client)
		: realClient((realClientType*)client), userinfo(init_userinfo()), netchan(init_netchan()), name(init_name()), state(init_state()), ping(init_ping()), rate(init_rate()), gentity(init_gentity())
	{
	}
...
{
#define IMPORT_FUNCTION(func_name, return_type, ...) virtual return_type func_name(__VA_ARGS__)=0; 

public:
	virtual void Printf(const char* format, ...) = 0;
	virtual void Printf(std::string& format, ...) = 0; // extra for convinence
	...
	IMPORT_FUNCTION(GetUserinfo, void, int index, char* buffer, int bufferSize);
...
public:
	...
	virtual void Printf(const char* format, ...) override
	{
		...
		realGameImport.Printf(fullstr);
	}
	virtual void Printf(std::string& format, ...) override
	{
	...
	virtual void DPrintf(const char* format, ...) override
	{
		...
		realGameImport.DPrintf(fullstr);
	}
	...
	virtual void centerprintf(gentity_t* ent, const char* format, ...)
	{
		...
		vsprintf(fullstr, format, va);
		va_end(va);
		(((void(*)(gentity_t *, const char*, ...))(realGameImport.centerprintf)))(ent, fullstr);
	}
	virtual void SendServerCommand(int client, const char* format, ...)//shouldn't exceed 500 bytes
	{
		...
		va_start(va, format);
		vsnprintf(fullstr, bufsz, format, va);
		va_end(va);
		realGameImport.SendServerCommand(client, fullstr);
	}
	virtual void Error(errorParm_t code, char* fmt, ...)
	{
		...
		vsprintf(fullstr, fmt, va);
		va_end(va);
		realGameImport.Error(code, fullstr);
	}
...
#define IMPORT_FUNCTION_OVERRIDE(func_name, return_type, parameter_list, parameter_names) virtual return_type func_name(parameter_list) {return realGameImport.func_name(parameter_names);}; 
	...
	IMPORT_FUNCTION_OVERRIDE(GetUserinfo, void, ECS(int index, char* buffer, int bufferSize), ECS(index, buffer, bufferSize));

#undef ECS
#undef IMPORT_FUNCTION_OVERRIDE
};
...
{
protected:
	gentity_t* const& gentities;
public:

#define EXPORT_SETTER_GETTER(func_name, return_type, ...) virtual void Set##func_name (return_type(*func_name)(__VA_ARGS__)) = 0; virtual return_type(*func_name()) (__VA_ARGS__) = 0; 
	...
	EXPORT_SETTER_GETTER(SetMap, void, char*);
	EXPORT_SETTER_GETTER(RunFrame, void, int svsTime, int frameTime);
	EXPORT_SETTER_GETTER(Shutdown, void, void);

#undef EXPORT_SETTER_GETTER
	int &num_entities;
	int &max_entities;
	int & apiVersion;
	BaseGameExport(gentity_t** gents, int& n_ents, int& max_ents, int &apiVer)
		: gentities(*gents), num_entities(n_ents), max_entities(max_ents), apiVersion(apiVer)
	{}
	virtual void* GetRealGameExport()=0;

	virtual GEntity GetGEntity(int ent_num)=0;
...
public:
	GameExport(T* rge)
		: realGameExport(*rge), BaseGameExport((gentity_t**)&rge->gentities, rge->num_entities, rge->max_entities, rge->apiVersion)
	{

	}
#define EXPORT_SETTER_GETTER_OVERRIDE(func_name, return_type, ...)\
 virtual void Set##func_name (return_type(*func_name)(__VA_ARGS__)) {realGameExport.func_name = (decltype(realGameExport.func_name))func_name;};\
 virtual return_type(*func_name()) (__VA_ARGS__) {return (return_type(*) (__VA_ARGS__))realGameExport.func_name;};
	...
	EXPORT_SETTER_GETTER_OVERRIDE(SetMap, void, char*);
	EXPORT_SETTER_GETTER_OVERRIDE(RunFrame, void, int svsTime, int frameTime);
	EXPORT_SETTER_GETTER_OVERRIDE(Shutdown, void, void);

#undef EXPORT_SETTER_GETTER_OVERRIDE
	//do not use for backup
	virtual void* GetRealGameExport()
	{
		return &realGameExport;
	}

	//ent_num must be < max_entities
	virtual GEntity GetGEntity(int ent_num)
	{
		return GEntity((gentity_t*)&((decltype(T::gentities))gentities)[ent_num]);
	}
};


template <class T>
class GameExportBackup : public BaseGameExport
{
	T realBackupGameExport;
public:
	GameExportBackup(T& rge)
		: realBackupGameExport(rge), BaseGameExport((gentity_t**)&rge.gentities, rge.num_entities, rge.max_entities, rge.apiVersion)
	{

	}
#define EXPORTBACKUP_SETTER_GETTER_OVERRIDE(func_name, return_type, ...)\
 virtual void Set##func_name (return_type(*func_name)(__VA_ARGS__)) {realBackupGameExport.func_name = (decltype(realBackupGameExport.func_name))func_name;};\
 virtual return_type(*func_name()) (__VA_ARGS__) {return (return_type(*) (__VA_ARGS__))realBackupGameExport.func_name;};

	EXPORTBACKUP_SETTER_GETTER_OVERRIDE(Cleanup, void, qboolean sameMap);
	EXPORTBACKUP_SETTER_GETTER_OVERRIDE(ClientBegin, void, gentity_t* ent, userCmd_t* cmd);
	EXPORTBACKUP_SETTER_GETTER_OVERRIDE(ClientCommand, void, gentity_t* ent);
	EXPORTBACKUP_SETTER_GETTER_OVERRIDE(ClientConnect, char*, int clientNum, qboolean firstTime, int a3);// different in sh/bt
	EXPORTBACKUP_SETTER_GETTER_OVERRIDE(ClientDisconnect, void, gentity_t* ent);
	EXPORTBACKUP_SETTER_GETTER_OVERRIDE(ClientThink, void, gentity_t* ent, userCmd_t* ucmd, userEyes_t* eyeInfo);
	EXPORTBACKUP_SETTER_GETTER_OVERRIDE(ClientUserinfoChanged, void, gentity_t* ent, char* userInfo);
	EXPORTBACKUP_SETTER_GETTER_OVERRIDE(ConsoleCommand, qboolean, void);
	EXPORTBACKUP_SETTER_GETTER_OVERRIDE(Init, void, int svsStartTime, int randomSeed);
	EXPORTBACKUP_SETTER_GETTER_OVERRIDE(SetMap, void, char*);
	EXPORTBACKUP_SETTER_GETTER_OVERRIDE(RunFrame, void, int svsTime, int frameTime);
	EXPORTBACKUP_SETTER_GETTER_OVERRIDE(Shutdown, void, void);

#undef EXPORTBACKUP_SETTER_GETTER_OVERRIDE
	//do not use for backup
	virtual void* GetRealGameExport()
	{
		return &realBackupGameExport;
	}
	//do not use for backup
	virtual GEntity GetGEntity(int ent_num)
	{
...
public:
	...
	static shared_ptr<BaseGameExport> GetGameExportBackup(void* export_actual)
	{
		if (gameInfo.IsAA())
		{
			return make_shared<GameExportBackup<gameExportAA_t>>(*(gameExportAA_t*)export_actual);
		}
		else if (gameInfo.IsSH())
		{
			return make_shared<GameExportBackup<gameExportDSH_t>>(*(gameExportDSH_t*)export_actual);
		}
		else if (gameInfo.IsBT())
		{
			return make_shared<GameExportBackup<gameExportDBT_t>>(*(gameExportDBT_t*)export_actual);
		}
	}
};
