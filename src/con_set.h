...
public:
	...
	con_set< key, value > &getConSet() const;
};
...
template<typename k, typename v>
inline Entry<k, v>**& con_set<k, v>::getTable() const
{
	return table;
}

template<typename k, typename v>
inline unsigned int con_set<k, v>::getTableLength() const
{
	return tableLength;
}
...
template<typename key, typename value>
inline con_set<key, value>& con_map<key, value>::getConSet() const
{
	return (con_set<key, value>&)m_con_set;
}
